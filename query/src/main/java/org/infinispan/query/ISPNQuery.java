...
public class ISPNQuery implements HSQuery, Serializable {
    ...
    * {@link #queryEntityInfos} or {@link #queryDocumentExtractor} is called.
    */
   ...
   public HSQuery targetedEntities(List<Class<?>> classes) {
      clearCachedResults();
      this.targetedEntities = classes == null ? new ArrayList<Class<?>>(0)
               : new ArrayList<Class<?>>(classes);
      final Class[] classesAsArray = targetedEntities.toArray(new Class[targetedEntities.size()]);
      this.indexedTargetedEntities = searchFactoryImplementor
               .getIndexedTypesPolymorphic(classesAsArray);
      if (targetedEntities != null && targetedEntities.size() > 0
               && indexedTargetedEntities.size() == 0) {
   ...
   private DocumentExtractor buildDocumentExtractor(IndexSearcherWithPayload searcher,
            QueryHits queryHits, int first, int max) {
      return new DocumentExtractorImpl(queryHits, searchFactoryImplementor, projectedFields,
               idFieldNames, allowFieldSelectionInProjection, searcher, luceneQuery, first, max,
   ...
   public QueryHits getQueryHits() {
      QueryHits queryHits;
      try {
         queryHits = getQueryHits(buildSearcher(), calculateTopDocsRetrievalSize());
      } catch (IOException e) {
         return null;
      }
      return queryHits;
   ...
   /**
    * DocumentExtractor returns a traverser over the full-text results (EntityInfo) This operation
    * is lazy bound: - the query is executed - results are not retrieved until actually requested
    * 
   ...
   public int queryResultSize() {
      if (resultSize == null) {
         ...
         if (searcher == null) {
            ...
            TopDocs hits;
            try {
               hits = getQueryHits(searcher, 1).getTopDocs(); // Lucene enforces that at least one
               // top doc will be retrieved.
   ...
   public Explanation explain(int documentId) {
      ...
      if (searcher == null) {
         throw new SearchException("Unable to build explanation for document id:" + documentId
                  + ". no index found");
      }
      try {
   ...
   /**
    * This class caches some of the query results and we need to reset the state in case something
    * in the query changes (eg a new filter is set).
    */
    ...
    * 
    * @return An instance of <code>QueryHits</code> wrapping the Lucene query and the matching
    *         documents.
    * 
    * @throws IOException
    *            in case there is an error executing the lucene search.
    */
   private QueryHits getQueryHits(IndexSearcherWithPayload searcher, Integer n) throws IOException {
      ...
      if (n == null) { // try to make sure that we get the right amount of top docs
         ...
         queryHits = new QueryHits(searcher, filteredQuery, filter, sort, n,
                  getTimeoutManagerImpl(), facetManager.getFacetRequests(), useFieldCacheOnTypes(),
                  getAppropriateIdFieldCollectorFactory());
      }
      resultSize = queryHits.getTotalHits();

      if (stats) {
         searchFactoryImplementor.getStatisticsImplementor().searchExecuted(
                  filteredQuery.toString(), System.nanoTime() - startTime);
      }
   ...
   /**
    * @return Calculates the number of <code>TopDocs</code> which should be retrieved as part of the
    *         query. If Hibernate's pagination parameters are set returned value is
    *         <code>first + maxResults</code>. Otherwise <code>null</code> is returned.
    */
   private Integer calculateTopDocsRetrievalSize() {
      if (maxResults == null) {
         ...
         if (tmpMaxResult == 0) {
            return 1; // Lucene enforces that at least one top doc will be retrieved. See also
            // HSEARCH-604
         } else {
    ...
    *           the search factory.
    * @param forceScoring
    *           if true, force SCORE computation, if false, force not to compute score, if null used
    *           best choice
    * 
    * @return the <code>IndexSearcher</code> for this query (can be <code>null</code>. TODO change
    *         classesAndSubclasses by side effect, which is a mismatch with the Searcher return, fix
    *         that.
    */
   private IndexSearcherWithPayload buildSearcher(
            SearchFactoryImplementor searchFactoryImplementor, Boolean forceScoring) {
      Map<Class<?>, DocumentBuilderIndexedEntity<?>> builders = searchFactoryImplementor
      ...
      // TODO check if caching this work for the last n list of indexedTargetedEntities makes a perf
      // boost
      if (indexedTargetedEntities.size() == 0) {
         ...
         for (Class clazz : involvedClasses) {
            ...
            if (builder == null) {
               throw new SearchException("Not a mapped entity (don't forget to add @Indexed): "
                        + clazz);
            }
            if (builder.getIdKeywordName() != null) {
               idFieldNames.add(builder.getIdKeywordName());
      ...
      // if at least one DP contains one class that is not part of the targeted classesAndSubclasses
      // we can't optimize
      if (classesAndSubclasses != null) {
         for (DirectoryProvider dp : targetedDirectories) {
            final Set<Class<?>> classesInDirectoryProvider = searchFactoryImplementor
                     .getClassesInDirectoryProvider(dp);
         ...
         Map<Class<?>, DocumentBuilderIndexedEntity<?>> documentBuildersIndexedEntities = searchFactoryImplementor
                  .getDocumentBuildersIndexedEntities();
         this.classesAndSubclasses = documentBuildersIndexedEntities.keySet();
      }

      // set up the searcher
      final DirectoryProvider[] directoryProviders = targetedDirectories
               .toArray(new DirectoryProvider[targetedDirectories.size()]);
      IndexSearcher is = new IndexSearcher(searchFactoryImplementor.getReaderProvider().openReader(
               directoryProviders));
   ...
   private void populateDirectories(List<DirectoryProvider> directories,
      ...
      final IndexShardingStrategy indexShardingStrategy = builder
      ...
      if (filterDefinitions != null && !filterDefinitions.isEmpty()) {
         directoryProviders = indexShardingStrategy.getDirectoryProvidersForQuery(filterDefinitions
                  .values().toArray(new FullTextFilterImplementor[filterDefinitions.size()]));
      } else {
         // no filter get all shards
         directoryProviders = indexShardingStrategy
                  .getDirectoryProvidersForQuery(EMPTY_FULL_TEXT_FILTER_IMPLEMENTOR);
      }
    ...
    *           the Hibernate specific <code>FullTextFilter</code> used to create the Lucene
    *           <code>Filter</code>.
    * 
   ...
   private Filter buildLuceneFilter(FullTextFilterImpl fullTextFilter) {
      ...
      /*
       * FilterKey implementations and Filter(Factory) do not have to be threadsafe wrt their
       * parameter injection as FilterCachingStrategy ensure a memory barrier between concurrent
       * thread calls
       */
      ...
      Filter filter = cacheInstance(def.getCacheMode()) ? searchFactoryImplementor
               .getFilterCachingStrategy().getCachedFilter(key) : null;
    ...
    * 
    * @param filter
    *           the filter which maybe gets wrapped.
    * @param def
    *           The filter definition used to decide whether wrapping should occur or not.
    * 
   ...
   private org.apache.lucene.search.Query filterQueryByClasses(
   ...
   private FieldCacheCollectorFactory getAppropriateIdFieldCollectorFactory() {
      ...
      for (Class<?> clazz : classesAndSubclasses) {
         ...
         FieldCacheCollectorFactory fieldCacheCollectionFactory = docBuilder
